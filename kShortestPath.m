function [shortestPaths, totalCosts] = kShortestPath(total_nodes,netCostMatrix, source, destination, k_paths)
...
% This function calls a slightly modified function dijkstra_multiple() 
...
if source > size(netCostMatrix,1) || destination > size(netCostMatrix,1)
    ...
    %---------------------INITIALIZATION---------------------
    k=1;
    [path cost] = dijkstra_simple(total_nodes,netCostMatrix, source, destination);
    ...
    if isempty(path)
        ...
        %***********************
        % K = 1 is the shortest path returned by dijkstra_multiple():
        ...
        while (k < k_paths   &&   size_X ~= 0 )
            ...
            for index_dev_vertex= w_index_in_path: length(P_) - 1   %index_dev_vertex is index in P_ of deviation vertex
                ...
                %call dijkstra_multiple between deviation vertex to destination node    
                [dev_p c] = dijkstra_simple(total_nodes,temp_netCostMatrix, P_(index_dev_vertex), destination);
                if ~isempty(dev_p)
